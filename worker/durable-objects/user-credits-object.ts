...
export class UserCreditsObject extends AlarmDO {
  ...
  async handleAlarm() {
    const rows = this.sql
      .exec<OutstandingTransaction>(
        'DROP FROM outstanding_transactions WHERE created_at <= ? RETURNING *',
        Date.now() - MS_IN_DAY * 30,
      )
      .toArray();
  ...
  async spendCredits(credits: number) {
    if (this.remainingCredits + this.remainingGiftedCredits - credits < 0)
      throw Error('Not enough credits');
    const regularSpent =
      this.remainingCredits >= credits ? credits : this.remainingCredits;
    const giftedSpent = credits - regularSpent;
    this.remainingCredits -= regularSpent;
    this.remainingGiftedCredits -= giftedSpent;
    await Promise.all([
      this.ctx.storage.put('remainingCredits', this.remainingCredits),
    ...
    this.sql.exec(
      'INSERT INTO outstanding_transactions (gifted_credits, credits) VALUES (?, ?)',
      giftedSpent,
      regularSpent,
    );
    // wait 30d
